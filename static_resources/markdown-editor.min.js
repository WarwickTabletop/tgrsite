...
b(a).before(u.append(r));g||u.find(".preview").slideUp();b('[data-tooltip="tooltip"]').tooltip();b(a).on("keydown",function(p){w.find(".card").addClass("text-muted");u.find("."+v).removeClass(v).addClass("fa-eye");h=!0;g&&(clearTimeout(x),x=setTimeout(z,600));return MarkdownHelper(a,p)});b("button",u).on("click",function(p){p.preventDefault();a.focus();var d=this.className.substr(16);p={start:a.selectionStart,end:a.selectionEnd};var e=n[d];if("preview"===d)return z();if("help"===d)return!0;var t=
toLines(a.value,p);d=t.lines;let c=t.selection;e===n.code&&c.start.line!==c.end.line&&(e=n.pre);let y=0;t=0;if(e.line)if(c.start.line===c.end.line&&""===d[c.start.line])d[c.start.line]=e.start+e.placeholder+e.end,t+=length(d[c.start.line]);else{y+=length(e.start);for(let A=c.start.line;A<=c.end.line;A++)d[A]=e.start+d[A]+e.end,t+=length(e.start)}else p.start!==p.end?(d[c.end.line]=d[c.end.line].substring(0,c.end.pos)+e.end+d[c.end.line].substring(c.end.pos),d[c.start.line]=d[c.start.line].substring(0,
c.start.pos)+e.start+d[c.start.line].substring(c.start.pos),y+=length(e.start),t+=length(e.start)):(d[c.start.line]=d[c.start.line].substring(0,c.start.pos)+e.start+e.placeholder+e.end+d[c.start.line].substring(c.start.pos),y+=length(e.start),t+=length(e.start+e.placeholder));e.block&&0<=c.start.line-1&&""!==d[c.start.line-1]&&(d[c.start.line]="\n"+d[c.start.line],y+=1,t+=1);e.block&&c.end.line+1<=d.length&&""!==d[c.end.line+1]&&(d[c.end.line]+="\n");a.value=d.join("\n");e=p.start+y;p=p.end+t;a.focus();
a.setSelectionRange?a.setSelectionRange(k(a,e),k(a,p)):a.createTextRange&&(d=a.createTextRange(),d.collapse(!0),d.moveEnd("character",p),d.moveStart("character",e),d.select());h=!0;w.find(".card").addClass("text-muted");u.find("."+v).removeClass(v).addClass("fa-eye");g&&(clearTimeout(x),x=setTimeout(z,600));return!0})})}})(jQuery,window,document);function createPreview(b,f,l){$.post(l,{md:b.value},function(q,g,k){f.find(".preview").html(q).slideDown()})}
