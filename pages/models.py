...
class Page(models.Model):
    ...
    name = models.CharField(max_length=64, blank=False,
                            help_text='Internal name of page')
    title = models.CharField(max_length=64, blank=True,
                             help_text='Page title to display in titlebar of browser/tab')
    ...
    css = models.TextField(max_length=16384, blank=True,
                           help_text='Custom CSS styles for the page')
    leftbar = models.TextField(
        max_length=16384, blank=True, help_text='Left sidebar contents (use cards)')
    markdown = models.BooleanField(
        default=True, help_text='Enable markdown rendering in this page')
    permission = models.IntegerField(
        choices=Permissions.choices, default=Permissions.PUBLIC)
...
class BreadcrumbParents(models.Model):
    url = models.CharField(max_length=2048, blank=False,
                           help_text='URL of parent')
    name = models.CharField(max_length=64, blank=False,
                            help_text='Name to display')
    order = models.IntegerField(
        blank=False, help_text='Order parents are to appear (lower is earlier)')
    page = models.ForeignKey(
        Page, blank=False, on_delete=models.CASCADE, related_name="breadcrumb_parents")
...
class Widget(models.Model):
    ...
    page = models.ForeignKey(
        Page, blank=False, on_delete=models.CASCADE, related_name="widgets")
